...
const checkoutView = async (req, res, next) => {
  try {
    if (req.session.user) {
      ...
      console.log(coupondetails, "This is the coupon details view...");

      const name = req.session.user.name;
      const userdetails = await User.findOne({ name });
      const cart = await Cart.findOne({ user: userdetails._id }).populate('product.productId');
...
const placeOrder = async (req, res, next) => {
  try {
    ...
    const discountprice = 10; // Update this value as per your requirement
...
const verifyPayment = async (req, res) => {
  try {
    ...
    if (hmac === req.body.payment.razorpay_signature) {
      await User.updateOne({ _id: req.session.user_id }, { $inc: { wallet: -subtotal } });

      const orderId = req.body.order.receipt;

      await Order.findByIdAndUpdate(orderId, { $set: { status: "COD" } });
      await Order.findByIdAndUpdate(orderId, { $set: { paymentId: req.body.payment.razorpay_payment_id } });
